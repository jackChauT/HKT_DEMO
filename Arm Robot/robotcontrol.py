...
# 测试函数
class AuboRobotService:
    def __init__(self, ip, port):
        print "init AuboRobotService"
        self.ip = ip
        self.port = port
    ...
    def exec_cmd(self, test_count):
        ...
        while test_count > 0:
            ...
            # 获取关节最大加速度
            logger.info(self.robot.get_joint_maxacc())

            # 正解测试
            fk_ret = self.robot.forward_kin((-0.000003, -0.127267, -1.321122, 0.376934, -1.570796, -0.000008))
            logger.info(fk_ret)

            # 逆解
            joint_radian = (0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000)
            ik_result = self.robot.inverse_kin(joint_radian, fk_ret['pos'], fk_ret['ori'])
            logger.info(ik_result)
            ...
            # 设置圆运动圈数
            self.robot.set_circular_loop_times(3)

            # 圆弧运动
            logger.info("move_track ARC_CIR")
            self.robot.move_track(RobotMoveTrackType.ARC_CIR)
    ...
    def start_pick(self, option = 0, timeout = 60): # 4 minutes timeout
        print "Requesting pick up call "
        r = False
        print "="*70
        print "="*70
        print "CALLING START PICK"
        print "="*70
        print "="*70
        self.on_mission = True
        # ROBOT ARM START
        self.robot.set_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_10, 1)
        
        # Drink Option
        self.robot.set_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_11, option)
        time.sleep(2)

        self.robot.set_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_10, 0)
        time.sleep(5)
        
        old_robot_state = self.robot.get_robot_state()

        # if stopped_robot_state_count > stopped_robot_status_limit, on_mission = false        
        stopped_robot_state_count = 1
        stopped_robot_status_limit = 10
        ...
        while time_pass < timeout:
            ...
            time_pass = int(t1 - t0)
            time.sleep(1)
            print "listening to out 11 ,", "Robot Status: ", self.robot.get_robot_state()

            if self.robot.get_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_12) == 1:
                print "Total Time: ", time_pass
                # finish_pick == True
                print "="*70
                print "finsihed print"
                print "="*70
                r = True
                self.on_mission = False 
                break
    ...
    def stop_robot_arm(self):
        try:
            ...
            print "Error stop_robot_arm: ", e
            

    def finish_pick(self, timeout = 30):
        # 循环测试
        # Listen:
        t0 = time.time()
        if timeout < 10:
        ...
        while time_pass < timeout:
            t1 = time.time()
            time_pass = int(t1 - t0)
            # robot.get_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_10)
            time.sleep(1)
            print "listening to out 11"
            if self.robot.get_board_io_status(RobotIOType.User_DO, RobotUserIoName.user_do_11) == 1:

            # print "listening to in 01"
            # if self.robot.get_board_io_status(RobotIOType.User_DI, RobotUserIoName.user_di_01) == 1:
                # finish_pick == True
                print "="*70
                print "finsihed print"
                print "="*70
    ...
    def connect_robot(self):
        ...
        # 启动测试
        logger.info("{0} connecting robot...".format(Auboi5Robot.get_local_time()))
        ...
        try:
            ...
            logger.error("{0} self.robot Event:{1}".format(self.robot.get_local_time(), e))
